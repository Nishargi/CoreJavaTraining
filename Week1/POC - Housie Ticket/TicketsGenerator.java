...
public class TicketsGenerator 
{
		...
		public int doColumnCount(int c) {
			int count = 0;
			for (int i = 0; i < 3; i++) {
				if (tickets[i][c] != 0)
					count++;
		...
		public int findEmptyCellInColumn(int c) {
			for (int i = 0; i < 3; i++) {
				if (tickets[i][c] == 0)
					return i;
			}
		...
		public void generateTicket() {
			...
			TicketsGenerator[] tickets = new TicketsGenerator[6];
			for (int i = 0; i < 6; i++) { tickets[i] = new TicketsGenerator(); }
			...
			for (int i = 0; i < 9; i++) 
			{
				...
				for (int j = 0; j < 6; j++) 
				{
					int index = generateRandomNumber(0, li.size() - 1);
			...
			int randNumIndex = generateRandomNumber(0, lastCol.size() - 1);
			int randNum = lastCol.get(randNumIndex);
			int randSetIndex = generateRandomNumber(0, sets.size() - 1);
			...
			for (int pass = 0; pass < 3; pass++) {
				for (int i = 0; i < 9; i++) {
					...
					int randNumIndex1 = generateRandomNumber(0, col.size() - 1);
					int randNums = col.get(randNumIndex1);
	
					boolean flag = false;
					while (!flag) {
						int randSetIndex1 = generateRandomNumber(0, sets.size() - 1);
			...
			for (int setIndex = 0; setIndex < 6; setIndex++) {
				...
				TicketsGenerator ticketNum = tickets[setIndex];
	
				// 1st Row
				for (int size = 3; size > 0; size--) {
					if (ticketNum.doRowCount(0) == 5)
						break;
			...
			for (int i = 0; i < 1; i++) 
			{
				TicketsGenerator currTicket = tickets[i];
				for (int r = 0; r < 3; r++) {
	  ...
	  public static void main(String[] args) {
		...
		TicketsGenerator t1 = new TicketsGenerator();
		t1.generateTicket(); }
}